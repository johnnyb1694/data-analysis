...
Now we are ready to fit a logistic regression to each individual term. Let \(U_t^{(w)}\) denote the 'relative usage' of word \(w\) at time $t$ - by 'relative usage' we mean how frequently word $w$ is used relative to all other words at the same time $t$. Then: 

$$
U_t^{(w)} = \dfrac{1}{1 + e^{-(\beta_0^{(w)} + \beta_1^{(w)} t )}} \in (0, 1)
$$

where $\beta_1^{(w)}$ can be interpreted as the *rate* of growth (or shrinkage, if negative) associated with word $w$. Indeed, this formula is on a per-word basis; we will fit a logistic growth model to each respective term in the input data via `stats::glm()`.
