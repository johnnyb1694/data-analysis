...
A more interesting comparison may involve the ratio of african american imprisonment in relation to *other* races:

```{r}
prison_wide <- prison_mutated %>% 
  spread(key = pop_category, value = avg_imp_rate) %>%
  mutate(racial_ratio = Black / Other)

ggplot() +
  geom_polygon(data = prison_wide, 
               mapping = aes(x = long, y = lat, group = group, fill = racial_ratio)) +
  scale_fill_gradient2(low = "slateblue2", midpoint = 7.5, high = "tomato2") +
  geom_label(data = prison_mutated %>% filter(pop_category == "Black", avg_imp_rate > 0.10) %>% distinct(region, .keep_all = TRUE),
             mapping = aes(label = region, x = long, y = lat)) +
  theme_void() +
  labs(fill = "Ratio of Black to Other",
       title = "African American imprisonment rates across the USA: 1970 - 2016",
       subtitle = "Racial discrimination appears to be more significant in Utah than Nebraska")
```
...
Now we can examine the evolution of imprisonment rates in these particular states:
...
I'm not so sure whether this quite worked out properly but it's my first time to use `gganimate` so there is a good chance I've made an error somewhere! Nonetheless, we are able to observe some spikes in imprisonment rates during the late 1990s in Utah and during the late 2000s in Nebraska too.
